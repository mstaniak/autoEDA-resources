...
load("usecase/compas/data/recid.rda")
...
The R output below is an example of `dataMaid` summary and checks.

```{r }
dataMaid::summarize(select(recid_small, age, race))
dataMaid::check(select(recid_small, age, race))
```
...
dlookr::diagnose(recid) %>%
  kable(caption = "Example dlookr output (truncated).") %>%
...
kable(autoeda_tbl[, 1:7], caption = "Example autoEDA output (truncated).") %>%
...
summary(arsenal::tableby(is_recid ~., data = recid_small)) %>%
  kable(caption = "Example arsenal output.") %>%
  kable_styling(full_width = FALSE)
```
